...
from tools.Option import CCOption, CCOptionType, CCOptionDepends, CCOptionDependsRelation

from tools.env import prepare_env

from tools.MenuSelection import MenuSelection
from tools.Menu import display_menu, ask_single
...
class TestCCOption(unittest.TestCase):

    def test_ccoption_from_json(self):
        test_case_ccoption()

    def test_ccoption_depends(self):
        dep_from_string = "another_option == another_value"
        depends = CCOptionDepends.from_string(dep_from_string)
        self.assertEqual(depends.option, "another_option")
        self.assertEqual(depends.value, "another_value")
        self.assertEqual(depends.relation, CCOptionDependsRelation.EQUAL)
